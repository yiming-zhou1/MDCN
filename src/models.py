...
class ResidualBlock(nn.Module):
    def __init__(self, inchannel, outchannel, kernel_size, stride, use_res, dropout):
        ...
        self.left = nn.Sequential(
            nn.Conv1d(inchannel, outchannel, kernel_size=kernel_size, stride=stride, padding=int(kernel_size / 2),
                      bias=False),
            nn.BatchNorm1d(outchannel),
            nn.Tanh(),
            nn.Conv1d(outchannel, outchannel, kernel_size=kernel_size, stride=1, padding=int(kernel_size / 2),
                      bias=False),
            nn.BatchNorm1d(outchannel)
        )
...
class MDCN(nn.Module):
    def __init__(self, global_para, substruct_para,substruct_num, global_dim, substruct_dim,vocab_size, ehr_adj, ddi_adj, ddi_mask_H, emb_dim=64, device=torch.device('cuda'),
                 ddi_in_memory=True,use_embedding=False):
        ...
        self.z = nn.Linear(256, vocab_size[2])

        self.tran = nn.Linear(4096, 153)

        ###########################################
        # 聚合单个visit内的diag和proc得到visit-level的表达F
        ...
        self.abc = nn.Linear(153, vocab_size[2])

        # self.ppp = nn.Linear(64,1)

        # self.residual = ResidualBlock(1, 64, 5, 1, True, 0.1)

        self.sample = nn.TransformerEncoderLayer(64, self.nhead)
        self.sample2 = nn.Linear(256, 131)
        self.sample3 = nn.Linear(64, 1)
    ###  Calculate the distance between different medical records
    ...
    def forward(self, input,mol_data,average_projection,substruct_data,ddi_mask_H):
        # input (adm, 3, codes)
        device = self.device
        ...
        ## we pay attention to the diagnosis and the procedure  separately, and then enter the distance network
        ...
        #  ---------------加----------------
        substruct_weight = torch.sigmoid(self.substruct_rela(query.squeeze()))
        ...
        encoded_medication = self.turn(molecule_embeddings.t())
        # ----------------加结束-----------------------------

        ##Graph coding of EHR and ddi knowledge to prepare for graph contrastive
        ...
        knowledge_graph = self.sample2(knowledge_graph.t())
        ehr_graph = self.sample2(ehr_graph.t())
        knowledge_graph = self.sample3(knowledge_graph.t())
        ehr_graph = self.sample3(ehr_graph.t())
        ...
        prob_g = prob_c_to_g.squeeze(dim=1)
