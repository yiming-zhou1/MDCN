...
from models import MDCN
from util import llprint, multi_label_metric, ddi_rate_score, get_n_params, buildPrjSmiles,graph_batch_from_smile
...
parser.add_argument('--dp', default=0.63, type=float, help='dropout ratio')
...
##Graph Contrastive Learning part
...
## evalution
def eval(model, data_eval, voc_size, epoch,molecule_forward,average_projection,substruct_forward,ddi_mask_H):
    # evaluate
    ...
    for step, input in enumerate(data_eval):
        ...
        for adm_idx, adm in enumerate(input):
            target_output1 = model(input[:adm_idx + 1],molecule_forward,average_projection,substruct_forward,ddi_mask_H)
    ...
    dill.dump(case_study, open(os.path.join('saved1', model_name, 'case_study.pkl'), 'wb'))
...
def main():
    if not os.path.exists(os.path.join("saved1", model_name)):
        os.makedirs(os.path.join("saved1", model_name))
    ...
    ddi_mask_path = 'E:\MDCN\data\ddi_mask_H.pkl'
    molecule_path = 'E:\MDCN\data\idx2SMILES.pkl'
    ddi_mask_H = dill.load(open(ddi_mask_path, 'rb'))
    molecule = dill.load(open(molecule_path, 'rb'))
    substruct_smile_path = 'E:\MDCN\data\substructure_smiles.pkl'
    data_path = r'E:\MDCN\data\records_final.pkl'
    voc_path = r'E:\MDCN\data\voc_final.pkl'
    ehr_adj_path = 'E:\MDCN\data\ehr_adj_final_newone.pkl'
    ddi_adj_path = 'E:\MDCN\data\ddi_A_final_new.pkl'
    ...
    model = MDCN(molecule_para,substruct_para,ddi_mask_H.shape[1],args.dim,args.dim,voc_size, ehr_adj, ddi_adj, ddi_mask_H, emb_dim=64,
    ...
    if TEST:
        eval(model, data_test, voc_size, 0,molecule_forward,average_projection,substruct_forward,ddi_mask_H)
    else:
        ...
        for epoch in range(EPOCH):
            ...
            for step, input in enumerate(data_train):
                for idx, adm in enumerate(input):
                    ...
                    target_output1, a, b = model(seq_input,molecule_forward,average_projection,substruct_forward,ddi_mask_H)
            ...
            ddi_rate, ja, prauc, avg_p, avg_r, avg_f1 = eval(model, data_eval, voc_size, epoch,molecule_forward,average_projection,substruct_forward,ddi_mask_H)
            ...
            torch.save(model.state_dict(), open(
                os.path.join('saved1', model_name, 'Epoch_%d_JA_%.4f_DDI_%.4f.model' % (epoch, ja, ddi_rate)), 'wb'))
            print('')
            if epoch != 0 and best_ja < ja:
                best_epoch = epoch
                best_ja = ja

        dill.dump(history, open(os.path.join('saved1', model_name, 'history.pkl'), 'wb'))

        # test
        torch.save(model.state_dict(), open(
            os.path.join('saved1', model_name, 'final.model'), 'wb'))
